...
void setup() {
  ...
  Serial.println("Trial Description: PRBS9");
...
void loop() {
   ...
   if (intboom == true){
    ...
    if (j>=10){
      if (i>=25){
      prbs_bit = (((prbs_reg >> 8) ^ (prbs_reg >> 4)) & 1);
      prbs_reg = ((prbs_reg << 1) | prbs_bit) & 0x1ff;
      motorSpeed = (bitRead(prbs_reg,8))*255;
      i = 0;
      }
      i++;
    }
