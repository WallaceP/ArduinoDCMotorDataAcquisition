...
unsigned int start = 0x02;
unsigned int prbs_reg = start;
...
void setup() {
  ...
  Serial.println("Trial Description: PRBS7");
  delay(10);
...
void loop() {
   ...
   if (intboom == true){
    ...
    if (j>=10 && stateMotorGo == true){
      if (i>=25){
      prbs_bit = (((prbs_reg >> 6) ^ (prbs_reg >> 5)) & 1);
      prbs_reg = ((prbs_reg << 1) | prbs_bit) & 0x7f;
      motorSpeed = (bitRead(prbs_reg,6))*255;
      i = 0;
      }
      i++;
    }
